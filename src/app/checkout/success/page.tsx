...
export default function CheckoutSuccessPage() {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        const oid =
          (est as any)?.order?.id != null ? Number((est as any).order.id) : null;
        ...
        if (isOrdered) {
          ...
          setStatus("done");
          if (oid && Number.isFinite(oid)) setOrderId(oid);
  ...
  if (!estimateId) {
    return (
      <div className="p-6 space-y-3">
        ...
        <p className="text-sm text-muted-foreground">Missing estimateId in URL.</p>
  ...
  return (
    <div className="p-6 space-y-3">
      ...
      {status === "done" && (
        <>
          <p className="text-sm text-muted-foreground">Order created successfully.</p>
          ...
          <div className="flex gap-2">
            ...
            {orderId ? (
              ...
              <Button onClick={() => router.replace(`/estimates/${estimateId}?paid=1`)}>
                Go to Estimate
              </Button>
            )}
      ...
      {status === "failed" && (
        <>
          ...
          <div className="flex gap-2">
            <Button onClick={() => router.replace(`/estimates/${estimateId}`)}>
              Open Estimate
            </Button>
