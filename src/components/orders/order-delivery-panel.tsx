...
export function OrderDeliveryPanel({
  ...
  const choosePickup = async () => {
    setBusy(true);
    try {
      await selectOrderPickup(order.id);
          ...
          <div key={delivery.id} className="rounded-lg border p-4">
            ...
            {isOwner &&
              ...
              delivery.status === "PAYMENT_DUE" && (
                ...
                >
                  <CreditCard className="h-4 w-4" /> Pay delivery ·{" "}
                  {formatMoney(Number(delivery.total))}
                </Button>
              )}
            ...
            {isPrivileged &&
              ...
              ["READY_TO_SCHEDULE", "SCHEDULED"].includes(delivery.status) && (
            ...
            {delivery.type === "INSTALLATION_OVERRIDE" &&
              delivery.status === "READY_TO_SCHEDULE" && (
                <p className="mt-4 rounded-md bg-blue-50 p-3 text-sm text-blue-800">
                  Paid. This delivery uses the accepted installation appointment
                  and completes automatically when installation starts.
                </p>
              )}
          </div>
        ))}
